...
笔者的开发环境是 WSL2, 在大部分情况下都可以很好的解决问题, 同时也可以使用 WMware Workstation Pro17 的虚拟机. 下面介绍两种方式, 笔者更倾向于第一种 WSL2 的便捷方式
...
需要注意的是, **配置信息的位置可能会变化, 所以可能你使用的 Linux 的版本的配置信息位置可能和笔者文中描述的不同!**

为了解决这个问题 menuconfig 中有一个比较方便的搜索功能, 输入 `/` 进入一个搜索页面, 可以在里面搜索关键字找到对应的配置位置

> 例如搜索 GDB 可以得到如下的一些结果, 根据结果中 Location 的信息找到对应的配置位置, 再进行修改
...
内核配置信息比较复杂, 笔者这里按经验总结了一些已有的配置选项, 适用于 linux6.6 版本

[linux6.6 内核配置文件](https://github.com/luzhixing12345/klinux/releases/tag/v0.0.2)
...
可以关闭一些诸如 文件系统支持, 设备驱动, 无线网络支持, USB 支持, 图形支持, 声音等
