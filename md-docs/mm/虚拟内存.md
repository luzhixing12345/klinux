...
CPU访问内存的最自然的方式就是使用物理地址, 这种方式被称为 **物理寻址**. 下图表示 CPU 读取从物理地址 4 开始的连续 4 个字节, 当 CPU 执行这条 load 指令时

1. 生成一个有效的物理地址和取址长度
2. 通过内存总线传递给主存
3. 主存根据地址找到物理地址为 4 的单元, 取出连续的 4 个字节, 并将其返回给 CPU
4. CPU 将其存放在一个寄存器之中

![20230326201733](https://raw.githubusercontent.com/learner-lu/picbed/master/20230326201733.png)

物理地址对于 CPU 来说非常方便，但是在这种情况下要想同时运行多个程序就非常困难了。如果第一个程序在0x2000的位置写入一个新的值，将会擦掉第二个程序存放在相同位置上的所有内容，所以需要程序员编程时非常小心的处理绝对地址。

操作系统是如何解决这个问题呢? 这里关键的问题是这两个程序都引引用了绝对物理地址，而这正是我们最需要避免的。我们可以把进程所使用的地址「隔离」开来，即让操作系统为每个进程分配独立的一套「虚拟地址」，虚拟地址彼此之间互不干涉，如果程序要访问虚拟地址，则由操作系统将其转换成不同的物理地址，这样不同的进程运行的时候，写入的则是不同的物理地址，也就不会产生冲突了

现代 CPU 普遍采用虚拟地址的寻址方式。CPU 通过生成一个虚拟地址(VA)来访问主存, 这个虚拟地址在被送到内存总线之前先传递到 CPU 芯片上的 **MMU** (Memory Management Unit)单元, **将一个虚拟地址转换成物理地址**, 在传输给内存。这一步需要 CPU 硬件和操作系统紧密结合, 如下图所示

![20230326202802](https://raw.githubusercontent.com/learner-lu/picbed/master/20230326202802.png)

> [!NOTE]
> 事实上 MMU 是 CPU 的硬件，它不是必须的。NoMMU 系统没有虚拟内存和地址隔离，程序直接访问物理内存。这类 CPU 往往被用在嵌入式、实时系统、单任务 firmware 中
> 
> 例如实时系统 RTOS 要求可预测性，而 MMU 引入：TLB miss，页错误，上下文切换开销；或者一些单进程系统如 bootloader
>
> 本系列的文章围绕有 MMU 的 CPU 展开讨论

## 物理地址空间和虚拟地址空间

系统中实际存在的内存空间是「物理地址空间」, 它表示当前系统中可用的内存容量

CPU 从一个 n 位地址空间中构建「虚拟地址空间」，它表示一个程序逻辑上可用的内存范围。

物理地址空间和虚拟地址空间的数值没有任何关系。物理内存实际上就是电脑上的内存条容量,一般是 8GB 16GB(2^34); 而虚拟地址空间则可以很大, 64 位 CPU 机器最多可以表示 2^64 = 16384P 范围的虚拟地址空间。
...
虚拟内存系统通过将虚拟内存分割称为虚拟页(Virtual Page, VP)的大小固定块, 每个虚拟页的大小是 P = 2^p 字节, 类似的物理内存也被分割为物理页(Physical Page, PP), 物理页大小和虚拟页一样都是P字节 , 物理页也可以称为页帧。Linux 与 Windows 都是采用「4kb」作为物理页和虚拟页大小
...
由于共享内存的存在, 多个虚拟地址可能映射到同一个物理地址, 在 [cache](./Cache和TLB.md) 的地址访问中讨论了关于 VIVT/PIPT/VIPT 三种情况的设计。**但是 TLB 不存在修改的情况**, **所以不存在别名和歧义**, 只是会多冗余了一些数据而已, 因此虽然**存在别名的情况**但是**不存在别名带来的问题**
...
- `PCD(Page Cache Disabled)`: 置为1表示disable,即该page中的内容是不可以被cache的。如果置为0(enable),还要看CR0寄存器中的CD位这个总控开关是否也是0
  ...
  该位大部分情况下为 1, 即默认启用 cache. 但是对于 [DMA](../arch/io.md) 以及 [VIVT 解决别名问题](./Cache和TLB.md) 中会关闭该位使得数据不经过 cache.
