...
前面我们讨论的访存流程中, CPU 得到虚拟地址, 需要先由 MMU 根据 PTBR 和 VPN 索引访问一次内存找到对应的 PTE, 在根据 PTE 中的地址找到对应的物理地址访存一次得到数据, 这个过程经历了两次访问内存
...
> 
> 注意此处和 [MESI](../arch/cc.md) 解决的问题不是一回事,因为当前不同的cacheline属于同一个CPU
...
> 
> 大部分情况下都是 cache enable, write back, 即 PTW=PCD=1
> 
