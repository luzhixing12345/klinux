...
处理器在进行存储器访问时,处理器访问的地址是虚拟地址,经过MMU的转换,得到物理地址. 那么查询cache组是用**虚拟地址还是物理地址**的Index域呢?当找到cache组时,使用虚拟地址还是物理地址的Tag域来匹配cache line呢?
...
我们先来看最简单的 PIPT 策略. 在 [虚拟地址转换](../mm/va_trans.md) 中我们介绍了虚拟地址转换为物理地址的顺序

- 检查 Transition Look Aside Buffer (TLB) 中的逻辑地址,如果 TLB 中存在该地址,请从 TLB 中获取页面的物理地址
- 如果不存在,请从物理内存访问页表,然后使用页表获取物理地址

所有这些时间都会增加命中时间. 因此对于 PIPT 的方式, 每一次查询时都需要完成 MMU 转换, 这个过程是相当连续的,因此**命中时间很长**, 每次对数据的 TLB 访问都会显著降低系统速度. 因此 PIPT 命中时间 = **TLB 延迟** + **缓存延迟**, 如果 TLB miss 则更慢了

一个直接的解决方案似乎是采用 VIVT 缓存.VIVT缓存直接检查缓存中的数据并获取数据, **CPU直接生成的地址可用于获取数据,从而大大缩短了命中时间**. 只有当数据不存在在缓存中时,才会检查TLB,最后在转换为物理地址后,数据才会被带入VIVT缓存.因此,VIVT 的命中时间 = 缓存命中时间

但是使用VIVT缓存也具有局限性

- TLB 包含重要标志, 如脏位和无效位,因此即使使用 VIVT 缓存,**仍然需要检查 TLB**
- **上下文切换**时大量缓存未命中:由于缓存特定于逻辑地址,并且每个进程都有自己的逻辑地址空间,因此两个进程可以使用相同的地址,但引用不同的数据. 对于每个上下文切换,都需要刷新缓存,并且每个上下文切换都会出现大量缓存未命中,这两者都非常耗时,并且会增加命中时间

因此解决这些问题的方法是虚拟索引物理标记缓存(**VIPT** 缓存). VIPT 缓存使用来自物理地址的标记位和索引作为来自逻辑/虚拟地址的索引.使用虚拟地址搜索缓存,并获取物理地址的标记部分.使用虚拟地址搜索 TLB,并获取物理地址.最后,将从VIPT缓存获取的物理地址的标签部分与从TLB获取的物理地址标签进行比较.如果它们都相同,则为缓存命中,否则为缓存未命中

TODO PIC

我们知道多个VA可能映射到同一PA,导致多个cache line组(VA不同,index域不同,查找到的cache 组则不同)映射到同一物理地址,这种现象叫做**cache alias**(高速缓存别名)

PIPT的数据cache一定不存在别名问题, 因为物理地址是唯一的,一个物理地址一定对应一个数据.但是不同的物理地址可能存储相同的数据.也就是说,物理地址对应数据是一对一关系,反过来是多对一关系.

VIVT/VIPT/PIPT的优缺点分析:

- VIVT:一旦一个VA到PA的映射关系改变,cache内容将会写回物理内存. 由于物理内存内容的变化需要同步到cache,就需要clean和invalidate(这两个操作结合起来就叫做flush)其余同名cache line,导致系统性能下降.
